<!DOCTYPE html>
<html lang="fr">
	<head>
		<meta charset="utf-8">
		<link rel="stylesheet" href="style.css">
		<link rel="stylesheet" href="bootstrap/css/bootstrap.css">
		<link href="https://fonts.googleapis.com/css?family=Sacramento" rel="stylesheet"> 
		<title></title>
	</head>
	
	<body>
		
			<div class="container">
			
			<header class="col-lg-offset-1 col-lg-10">
				<?php include ("include/header.php");?>
			</header>
			
			<section class="row col-lg-offset-1 col-lg-10 col-xs-12">
			
				<article class="presentation">
					<h1 class="pull-right">JE REALISE LE REPORTAGE DE VOTRE MARIAGE </h1>
					<p class="pull-right">Création originales pour des Photos qui vous ressemblent...</p>
					<img src="images/mariage/banniere.jpg" class="hidden-xs col-lg-12">
				</article>
				
				
				<aside class="col-lg-12 col-xs-12" >
					<div><a href="#" class="col-lg-offset-1 col-lg-4 col-xs-12 lien">Organiser son Mariage </a><div class="hidden-xs col-lg-1 fleche_droite"></div></div>
					<div><a href="#" class="col-lg-offset-1 col-lg-4 col-xs-12  lien">Consulter les Tarifs</a><div class="hidden-xs col-lg-1 fleche_droite"></div></div>
					<div><a href="#" class="col-lg-offset-1 col-lg-4 col-xs-12 lien">Découvrir la Galerie Photo</a><div class="hidden-xs col-lg-1 fleche_droite"></div></div>
					<div><a href="#" class="col-lg-offset-1 col-lg-4 col-xs-12  lien">Réserver une Date</a><div class="hidden-xs col-lg-1 fleche_droite"></div></div>
				</aside>
			</section>
		</div>
			
			
			<footer class="col-lg-offset-1 col-lg-10">
				<?php include ("include/footer.php");?>
			</footer>
			
			</div>
		
		<script src="bootstrap/js/bootsrap.js"></script>
		<script src="bootstrap/js/jquery.js"></script>
		<script src="bootstrap/js/jquery-3.1.1.js"></script>
    
	</body>
	
</html>